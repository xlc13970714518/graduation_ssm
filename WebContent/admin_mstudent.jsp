...
<script>
    ...
    var total;  
              //总条数
 function loadData(){
    	      $.post("admin_mstudent",{"pageIndex":page,"pageSize":limit},function(list){ 
                console.log(list);
               for(var i=0;i<list.length;i++){
                    var td = $("<td><input type='checkbox' class='checkbox' /></td>");
                    var td1= $("<td></td>").text(list[i].number);
                    var td2 = $("<td></td>").text(list[i].name);
                    var td3 = $("<td></td>").text(list[i].sex);
                    var td4 = $("<td></td>").text(list[i].major);
                    var td50 =$("<td></td>").text(list[i].dept);
                    var td5 = $("<td></td>").text(list[i].enyear);
                    var td6 = $("<td></td>").text(list[i].phonenumber);
                    var td7 = $("<td></td>").text(list[i].qq);
                    var tr = $("<tr></tr>").append(td,td1, td2, td3, td4,td50,td5,td6,td7);
                    $("tbody").append(tr);   
 ...
 function delete0(num) {
	 ...
	 $.ajax({
         type:"post",
         url:"deleteStudent",//对应controller的URL
         async:false,
         dataType: "json",
         data:{"number":num,},
...
function isadd() {
	window.location.href="admin_addSudent.jsp"; 
}
